...
# import os
# from .forms import UploadFileForm
# from .serializers import HealthDataSerializer, AiModelsSerializer  # تأكد من استخدام AiModelsSerializer فقط
# from .models import HealthData, AiModels, Disease, Diagnosis
#
#
# # دالة لتسجيل الدخول عبر HTML Template
...
from .models import HealthData, AiModels, AIModel  # استيراد النماذج
...
def import_data_from_airtable(request):
    headers = {
    ...
    if response.status_code == 200:
        ...
        for record in data["records"]:
            ...
            # إدخال البيانات في قاعدة بيانات Django
        ...
        return JsonResponse({"status": "Error fetching data from Airtable"}, status=400)
...
# Health Data List and Create View
class HealthDataListCreateView(generics.ListCreateAPIView):
    queryset = HealthData.objects.all()
    serializer_class = HealthDataSerializer


# Health Prediction API View
class HealthPredictionAPIView(APIView):
    def post(self, request, *args, **kwargs):
        prediction_result = your_prediction_function(request.data)
...
def upload_files_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                # استدعاء دالة لمعالجة البيانات
                # مثال: process_uploaded_data(df)
    ...
    return render(
        request,
        "healthdata_changelist.html",
        {"form": form, "message": message, "file_url": file_url},
    )


# AI Models API View (Create)
class AiModelsAPIView(APIView):
    def post(self, request, *args, **kwargs):
        serializer = AiModelsSerializer(data=request.data)
        if serializer.is_valid():
            if AiModels.objects.filter(name=request.data["name"]).exists():
                return Response(
            ...
            serializer.save()
            return Response(
                {"message": "Model added successfully!"}, status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)


# AI Models List and Create View (combined)
class AiModelsListCreateView(generics.ListCreateAPIView):
    queryset = AiModel.objects.all()  # استرجاع جميع النماذج
    serializer_class = AiModelSerializer  # تعيين المحول


# Example API View (For demonstration)
class ExampleAPIView(APIView):
    def get(self, request, *args, **kwargs):
        try:
            data = some_data_function()
            return Response(
...
def home_view(request):
    # استدعاء البيانات المطلوبة أو التعامل مع المنطق
    return render(
        request, "home.html"
    )  # تأكد من أن القالب home.html موجود في مجلد القوالب
